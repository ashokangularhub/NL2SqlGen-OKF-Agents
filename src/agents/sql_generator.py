...
class SQLGeneratorAgent(BaseAgent):
    ...
    _SYSTEM = (
        ...
        "  16. **NEVER fabricate an id/sku/code literal from a schema-doc EXAMPLE**: the schema context\n"
        "       may show a sample value purely to illustrate a FORMAT, e.g. `product_id` VARCHAR(20) PK\n"
        "       e.g. `AUR-EB-PRO2` or `sku` e.g. `AUR-EB-PRO2-BLK` — these are documentation samples for\n"
        "       ONE unrelated example product, NOT real data, and must NEVER be copied into a WHERE clause\n"
        "       just because the user's product happens to be mentioned nearby or the format looks plausible.\n"
        "       If the user refers to a product/customer/item by NAME or description rather than an exact\n"
        "       id/sku/code they typed themselves, filter on the descriptive column instead and let the\n"
        "       database resolve the concrete id through a JOIN:\n"
        "       ✅ CORRECT: SELECT order_items.sku, ... FROM customer_support.order_items\n"
        "                     JOIN customer_support.product_variants ON product_variants.sku = order_items.sku\n"
        "                     JOIN customer_support.products ON products.product_id = product_variants.product_id\n"
        "                     WHERE orders.order_id = 'ORD-2026-00843'\n"
        "                       AND products.product_name ILIKE '%AuroraSound Go%'\n"
        "       ❌ WRONG: ... WHERE product_variants.sku = 'AUR-EB-PRO2'\n"
        "                 (fabricated — that literal is products.md's PK example for a DIFFERENT product,\n"
        "                  \"AuroraBuds Pro 2\"; the user asked about a speaker, not earbuds)\n"
        "       An id/sku/order literal is only safe to use verbatim when the user's OWN message contains\n"
        "       that exact literal (e.g. an order id like 'ORD-2026-00843' the user typed is fine to use\n"
        "       directly — it did not come from a doc example).\n\n"
        ...
        "     the schema prefix.\n\n"
        "If validator feedback says an id/sku/code literal was fabricated (rule 16):\n"
        "  1. Do NOT swap it for a different guessed literal — you have no real data access.\n"
        "  2. Rewrite the filter on the descriptive column instead (e.g. products.product_name\n"
        "     ILIKE '%<name from user query>%') and JOIN through to the id/sku column so the\n"
        "     database resolves the concrete value itself."
    )
