...
class SQLValidatorAgent(BaseAgent):
    ...
    _SYSTEM = (
        ...
        "  7. FABRICATED ID/SKU LITERALS: if the user's message refers to a product/customer/item\n"
        "     by NAME or description (not an exact id/sku/code they typed themselves), the WHERE\n"
        "     clause must NOT filter on an exact products.product_id / product_variants.sku / similar\n"
        "     code literal — that value cannot have come from anywhere but a guessed/hallucinated\n"
        "     schema-doc example. Require a name-based filter instead (e.g. products.product_name\n"
        "     ILIKE '...') joined through to resolve the concrete id/sku. An id/sku/order literal IS\n"
        "     fine when the user's own message contains that exact literal verbatim.\n\n"
        "PASS if:\n"
        ...
        "  • WHERE filters on an exact id/sku/code literal for a product/customer/item the user only\n"
        "    named/described (rule 7) — feedback must say to filter by name instead and JOIN to resolve\n"
        "    the concrete id/sku, and must NOT suggest a different literal id/sku value\n\n"
